...
export function MinimalTemplate({ noPdfId }: Readonly<{ noPdfId?: boolean }> = {}) {
  ...
  const { accentColor, headerBg, density, photoShape } = theme
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18 }}>
          {personal.photo && photoShape !== 'none' && (
            <img
              ...
              style={{ width: 72, height: 72, borderRadius: photoRadius, objectFit: 'cover', border: '2px solid rgba(255,255,255,0.3)', flexShrink: 0 }}
            />
          )}
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
              <div>
                {personal.fullName && (
                  <h1 style={{ fontSize: 24, fontWeight: 700, lineHeight: 1.15, letterSpacing: '-0.01em' }}>{personal.fullName}</h1>
                )}
                {personal.jobTitle && (
                  <p style={{ fontSize: 12, color: '#93c5fd', marginTop: 3, fontWeight: 500 }}>{personal.jobTitle}</p>
                )}
              </div>
              {contacts.length > 0 && (
