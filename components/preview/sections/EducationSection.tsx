        ...
        {visible.map((edu) => (
          <div key={edu.id} className="page-break-avoid">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
              ...
              <p style={{ fontSize: 10.5, color: '#6b7280', flexShrink: 0, whiteSpace: 'nowrap' }}>
                {formatDateRange(edu.startDate, edu.endDate, false)}
              </p>
            </div>
            <p style={{ fontSize: 11, color: accentColor, fontWeight: 600, marginTop: 1 }}>
              {edu.institution}
              {edu.location && (
            ...
            {edu.description && (
              <p style={{ fontSize: 11, color: '#374151', lineHeight: 1.55, marginTop: 3 }}>
                {edu.description}
              </p>
            )}
          </div>
